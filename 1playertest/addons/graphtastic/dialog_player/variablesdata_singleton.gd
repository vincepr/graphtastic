...
signal GT_all_choices_disabled
...
var pictures: Dictionary = {
	"background": "res://mockupProject/Hills Free (update 3.0).png",
	"player": {"happy": "res://mockupProject/Hills Free (update 3.0).png", "angry": "res://mockupProject/Hills Free (update 3.0).png",}
...
func change_variable_to(key : String, expr : String):
	if key in variables:
		...
		return "cant change variable, not in variables "+ key + "by" + expr

func check_if(tocheck : String):
	#print("checking if: " +String(tocheck))
	if tocheck == "" or tocheck == " " or tocheck == "   ":
		return true
	var expression = Expression.new()
	var variables_pool= PoolStringArray (variables.keys())
...
func _on_GT_all_choices_are_disabled():
