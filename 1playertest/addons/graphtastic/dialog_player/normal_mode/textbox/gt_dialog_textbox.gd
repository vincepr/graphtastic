...
func setup_signals():
	...
	_err = connect("GT_custom_signal", GTP, "_on_GT_custom_signal")
	_err = connect("GT_preparing_chapter_change_to", GTP, "_on_GT_preparing_chapter_change_to")
...
func do_chapter(txt:String):
	...
	if split.size()<2:
		return false
	var next=[story_helper.set_startpoint(GTP.parse_expr(split[0])as int, GTP.parse_expr(split[1])as int)]
	if !next: return false
	next_graphs=[{"next_nID": GTP.parse_expr(split[1])as int}]
	emit_signal("GT_preparing_chapter_change_to",GTP.parse_expr(split[0])as int,GTP.parse_expr(split[1])as int)
...
func _parse_custom_signals(txt: String) -> String:
	...
	for _i in range(variable_count):
		...
		else: value = GTP.parse_expr(splitcommand[1].trim_suffix("}"))
...
func _parse_if_conditions(txt: String):
	...
	for _i in range(variable_count):
		...
		if splitcommand.size()==1: 
			...
			txt.erase(start_index, end_index - start_index)
		elif GTP.parse_expr(splitcommand[0]):
